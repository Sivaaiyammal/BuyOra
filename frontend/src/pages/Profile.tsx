...
const Profile = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await axios.put(`http://localhost:5000/api/profile/by-email/${email}`, formDataToSend, {
        headers: {
          ...
          <form onSubmit={handleSubmit}>
              ...
              <div className="md:w-1/4 flex flex-col items-center">
                <div className="relative">
                  <div className="w-32 h-32 rounded-full overflow-hidden bg-blue-100 border-4 border-white shadow-lg">
                    <img
                      src={formData.avatar ? formData.avatar.startsWith("blob:") ? formData.avatar : `http://localhost:5000${formData.avatar}` : "https://via.placeholder.com/150"}
                      alt="Profile"
                  ...
                  <input
                    ...
                    id="profile-upload"
                  />
                  <label htmlFor="profile-upload" className="absolute bottom-0 right-0 w-8 h-8 rounded-full bg-blue-500 text-white flex items-center justify-center shadow-md hover:bg-blue-600 transition-colors cursor-pointer">
                    <Pencil size={16} />
                  </label>
                </div>
              </div>
            ...
            <div className="mt-8 flex justify-end space-x-4">
              ...
              <button
                type="submit"
                disabled={saving}
                className={`px-6 py-2 rounded-lg text-white transition-colors ${saving ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-500 hover:bg-blue-600'
                  }`}
              >
                {saving ? 'Saving...' : 'Save'}
              </button>
