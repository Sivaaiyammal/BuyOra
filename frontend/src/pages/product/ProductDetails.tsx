...
const ProductDetails = () => {
  ...
  const handleRequest = (size: string) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm p-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div>
          <div className="mb-6">
            <h1 className="text-2xl font-bold text-gray-900">ID: {product.id}</h1>
            <h2 className="text-xl font-semibold mt-2">Product Name: {product.name}</h2>
            <p className="text-lg mt-2">Brand Name: {product.brand}</p>
            <p className="text-gray-600 mt-2">{product.category} {product.age}</p>
            <p className="text-gray-600">Item Weight: {product.weight}</p>
          </div>

          <div className="mb-6">
            <p className="text-lg text-gray-500 line-through">Original Price: ₹{product.originalPrice}</p>
            <p className="text-2xl font-bold text-gray-900">Offer Price: ₹{product.offerPrice} Only</p>
          </div>
              ...
              {product.sizes.map(size => (
                <div key={size.name} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-2">
                    <button
                      onClick={() => handleQuantityChange(size.name, (quantities[size.name] || 0) - 1)}
                    ...
                    <input
                      ...
                      onChange={(e) => handleQuantityChange(size.name, parseInt(e.target.value) || 0)}
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            ...
            <p className="text-gray-600 mb-2">Sky Blue Tailored Jacket, Floral Printed Dress, Comfort Meets Style</p>
            <p className="text-gray-600 mb-4">Perfect For: Special occasions, family celebrations, photo shoots, or as a standout gift.</p>
